...
export default {
  ...
  computed: {
    ...
    tableHeader() {
      let currentState =
        this.dataCollection && this.dataCollection[this.collectionKey];
      return (
        (currentState && currentState.tableheader) || [{ label: "", prop: "" }]
      );
    },
    isPagination() {
      let currentState =
  ...
  methods: {
    ...
    handleExpandChange(row, expandedRows, ...args) {
      ...
      let expandRowState = this.dataCollection && this.dataCollection[this.collectionKey + "_" + row[this.nextLevelIdName]];
      this.initComponent(expandRowState);
      if (!row.isCache) expandRowState.getData(row, row[this.idName]);
      row.isCache = true;
